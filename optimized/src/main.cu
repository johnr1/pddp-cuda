...
int main(int argc, char* argv[]) {
    ...
    dim3 dimGrid((M.cols/GRID_X)/2+1, M.rows/GRID_Y+1, 1);
    ...
    initialize<<<d_w.rows/S_BLOCK_SIZE + 1, S_BLOCK_SIZE>>>(d_w,0);             //Populates d_w
    calculateAverageVector<<<dimGrid, dimBlock>>>(d_M,d_w); //Populates d_w
    cudaCheckError();

    initialize<<<d_x.rows/S_BLOCK_SIZE + 1, S_BLOCK_SIZE>>>(d_x,1);             //Populates d_x
    cudaCheckError();
    ...
    print_to_file(x, output_file);

    // Print time elapsed
    cudaEventSynchronize(stop);
    float time_elapsed = 0;
    cudaEventElapsedTime(&time_elapsed, start, stop); //ms
