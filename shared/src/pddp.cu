...
__global__ void calculateAverageVector(Matrix M, Matrix w) {
    ...
    if(row >= M.rows)
        return;
    if (col + blockDim.x< M.cols)
        sh[yid][tid] = M.matrix[row * M.cols + col] +  M.matrix[row * M.cols + col + blockDim.x]; 
    else if(col < M.cols)
...
__global__ void subtractAndMultiply(Matrix M, Matrix w, Matrix x, Matrix temp) {
    ...
    if (col + blockDim.x< M.cols)
        sh[yid][tid] = (M.matrix[row * M.cols + col] - w.matrix[row]) * x.matrix[col] + 
            (M.matrix[row * M.cols + col + blockDim.x] - w.matrix[row]) * x.matrix[col+blockDim.x]; 
    else if(col < M.cols)
        sh[yid][tid] = (M.matrix[row * M.cols + col] - w.matrix[row]) * x.matrix[col]; 
    else
        sh[yid][tid] = 0;
...
void calculateNorm(Matrix x, Matrix *temp, Matrix *temp2, double* varianceNorm) {
    int blockSize = x.rows / S_BLOCK_SIZE / 2 + 1;
    ...
    while(blockSize > 1) {
        ...
        blockSize = blockSize/threads/2 + 1;
...
__global__ void normCalculationKernel(Matrix in, Matrix out, int limit, double* varianceNorm) {
    ...
    if (i + blockDim.x< limit)
        sh[tid] = in.matrix[i] * in.matrix[i] + in.matrix[i+blockDim.x] * in.matrix[i+blockDim.x];
    else if(i < limit)
